        ...
        <template v-if="deployMechanism === 'upload'">
          <div class="editor-stack">
            <div class="editor-block">
              <div class="inner-section-title">源码 ZIP 包</div>
              ...
              >
                ...
                <template #tip>
                  <div class="el-upload__tip upload-tip">
                    只能上传 .zip 文件，且一次仅允许一个文件；压缩包内必须包含 Dockerfile
                  </div>
                </template>
              </el-upload>
            ...
            <div class="editor-column single-column">
              ...
              <div class="editor-rendered-tip">
                以下内容为基于当前 Docker Command 模板渲染后的结果，可继续手动调整。Dockerfile 直接使用源码 ZIP
                中提供的内容。
              </div>
    ...
    >
      <el-form label-width="96px">
        <el-form-item label="模板名称">
          <el-input
            v-model="newDockercommandTemplate.template_name"
            placeholder="例如：Python 标准启动命令模板"
          />
        </el-form-item>

        <el-form-item label="描述">
          <el-input
            v-model="newDockercommandTemplate.description"
            placeholder="模板说明，可选"
          />
        </el-form-item>
        ...
        <el-form-item label="模板内容">
          <div class="dialog-editor-wrap">
            <CodeEditorBox
              v-model="newDockercommandTemplate.content"
              :min-rows="16"
              :max-rows="16"
            />
          </div>
      ...
      <template #footer>
        <button
          ...
          @click="createDockercommandTemplateDialogVisible = false"
        >
          取消
        </button>
        <button
...
async function loadDockercommandTemplates() {
  ...
  try {
    ...
    dockercommandTemplates.value = (list || []).filter((item: Template) => {
      return item.template_type === 'dockercommand' && item.project_type === 'python'
    })
