...
@project_router.get(
    "/api/deploy-agent/project/list",
    summary="获取项目数据",
    description="返回所有项目数据。"
)
    ...
    try:
        ...
        if permissions is not None:
            projects = [
                ...
                if project.get("project_code") in permissions
            ]

        return HttpResult.ok(data=projects)
    except FileNotFoundError:
        return HttpResult.fail(code=404, msg="project_data.json not found.")
    except Exception as e:
        return HttpResult.fail(code=500, msg=f"获取项目列表失败: {str(e)}")


@project_router.get(
    "/api/deploy-agent/project",
    summary="获取项目详情",
    description="根据项目 ID 获取对应的项目详细信息"
)
async def get_project(id: str = Query(..., description="项目 ID")):
    project = PROJECT_DATA_MANAGER.get_project(id)
    if project is not None:
        return HttpResult.ok(data=project)
    return HttpResult.fail(code=404, msg=f"未找到 ID 为 {id} 的项目")


@project_router.get(
    "/api/deploy-agent/project/support/render-template-content",
    summary="渲染模板内容, 渲染dockerfile和dockercommand"
)
async def render_template_content(
    ...
    try:
        ...
        return HttpResult.ok(data=content)
    except Exception as e:
        return HttpResult.fail(code=500, msg=f"渲染模板内容失败: {str(e)}")
...
async def check_web_project_deployment_status(id: str = Query(..., description="项目ID")):
    ...
    project = PROJECT_DATA_MANAGER.get_project(id)
    if project is None:
        return HttpResult.fail(code=404, msg="项目不存在")

    container_path = project.get("container_project_path")
    if not container_path:
        return HttpResult.fail(code=400, msg="项目未配置 container_project_path（容器项目路径）")
...
@project_router.get(
    "/api/deploy-agent/project/web/check-accessibility",
    summary="检测前端项目是否可访问",
    description="根据项目 access_url 判断是否可达"
)
async def check_web_project_accessibility(url: str = Query(..., description="前端项目的访问地址")):
    if not url or not url.strip():
        return HttpResult.fail(code=400, msg="url不能为空")
...
@project_router.post(
    "/api/deploy-agent/project/web/add",
    summary="创建待部署的 Web 项目"
)
...
@project_router.put(
    "/api/deploy-agent/project/web/update",
    summary="更新 Web 项目"
)
async def update_web_project(update_dto: UpdateWebProjectRequestDto):
    PROJECT_DATA_MANAGER.update_project(
        update_dto.id,
        update_dto.model_dump(exclude={"id"})
    )
    return HttpResult.ok(msg="更新成功")
...
async def deploy_web_project(
    ...
    try:
        ...
        if project is None:
            return HttpResult.fail(code=404, msg=f"没有 id 为 {project_id} 的 Web 项目")
        ...
        import traceback
        traceback.print_exc()
        return HttpResult.fail(code=500, msg=f"提交部署任务失败: {str(e)}")


@project_router.delete(
    "/api/deploy-agent/project/web/delete/{id}",
    summary="删除 Web 项目"
)
async def delete_web_project(id: str):
    PROJECT_DATA_MANAGER.delete_project(id)
    return HttpResult.ok(msg="删除成功")


# ==================== Java 项目接口 ====================

@project_router.get(
    "/api/deploy-agent/project/java/get/{id}",
    summary="获取 Java 项目详情"
)
async def get_java_project(id: str):
    project = PROJECT_DATA_MANAGER.get_project(id)
    if project is not None:
        return HttpResult.ok(data=project)
    return HttpResult.fail(code=404, msg=f"没有 id 为 {id} 的项目")


@project_router.post(
    "/api/deploy-agent/project/java/add",
    summary="创建待部署的 Java 项目"
)
...
@project_router.put(
    "/api/deploy-agent/project/java/update",
    summary="更新 Java 项目"
)
...
async def deploy_java_project(
    ...
    try:
        project = PROJECT_DATA_MANAGER.get_project(project_id)
        if project is None:
            return HttpResult.fail(code=404, msg=f"没有 id 为 {project_id} 的 Java 项目")
...
@project_router.delete(
    "/api/deploy-agent/project/java/delete/{id}",
    summary="删除 Java 项目"
)
...
@project_router.get(
    "/api/deploy-agent/project/python/get/{id}",
    summary="获取 Python 项目详情"
)
...
@project_router.post(
    "/api/deploy-agent/project/python/add",
    summary="创建待部署的 Python 项目"
)
...
@project_router.put(
    "/api/deploy-agent/project/python/update",
    summary="更新 Python 项目"
)
...
async def deploy_python_project(
    ...
    try:
        project = PROJECT_DATA_MANAGER.get_project(project_id)
        if project is None:
            return HttpResult.fail(code=404, msg=f"没有 id 为 {project_id} 的 Python 项目")
...
@project_router.delete(
    "/api/deploy-agent/project/python/delete/{id}",
    summary="删除 Python 项目"
)
